# Umbrella Worldwide ESLint Config

[![npm version](https://badge.fury.io/js/%40umbrellaworldwide%2Feslint-config.svg)](https://badge.fury.io/js/%40umbrellaworldwide%2Feslint-config)

Pairs well with our [Prettier config](https://www.npmjs.com/package/@umbrellaworldwide/prettier-config).

## Table of Contents

- [Umbrella Worldwide ESLint Config](#umbrellaworldwide-eslint-config)
...
Run `npm info "@umbrellaworldwide/eslint-config@latest" peerDependencies` to list the peer dependencies and versions.
...
2. Install dependencies
    ...
    - **Option 1:** With `npx`
      ...
      ```sh
      npx install-peerdeps --dev @umbrellaworldwide/eslint-config
      ```
...
{
  "root": true,
  "extends": "@umbrellaworldwide/eslint-config/four-spaces"
}
```
...
{
  "root": true,
  "extends": "@umbrellaworldwide/eslint-config/react"
}
```
...
1. Extend the base config (`react-app`) in your ESLint configuration:
   ...
   {
     "root": true,
     "extends": ["react-app", "@umbrellaworldwide/eslint-config/react"]
   }
   ```
...
{
  "root": true,
  "extends": "@umbrellaworldwide/eslint-config/vue"
}
```
...
Umbrella Worldwide's ESLint config extends `eslint:recommended` which enables rules that report common problems, which are marked with check marks in the large [list of ESLint rules](https://eslint.org/docs/rules/).
...
(*) Based on @upstatement/eslint-config.
